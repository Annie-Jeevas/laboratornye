...
public class UserFilter implements Filter {
    ...
    private void doBeforeProcessing(RequestWrapper request, ResponseWrapper response)
        ...
        if ((((HttpServletRequest) request).getSession().getAttribute("current_user_name") == null) 
            && 
            (((HttpServletRequest) request).getSession().getAttribute("current_user_role") == null)) {
            String username = ((HttpServletRequest) request).getRemoteUser();
            if (username != null) {
                try {
                    User user = new DAOUser().getUserByUsername(username);
                    ((HttpServletRequest) request).getSession().setAttribute("current_user_name", user.getLogin());
                    ((HttpServletRequest) request).getSession().setAttribute("current_user_role", user.getRole());
